...
If a companies-with-reg-info-<SEARCH_ARGS> file exists then this script will use that (e.g. if you've run the second script)
...
def read_command_line_args():
    ...
    input_file = f"output/companies-with-reg-info-{sys.argv[1]}.json"
    alternative_input_file =  f"output/companies-{sys.argv[1]}.json"
...
def create_html(suspect_companies, table_title):
    ...
    export_html_table_filename = f"output/results-{sys.argv[1]}.html"
...
def create_csv(suspect_companies):
    ...
    export_csv_filename = f"output/results-{sys.argv[1]}.csv"
